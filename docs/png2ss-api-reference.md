...
| `metadata_format` | string | no | `none` | Set to `json_array` to receive a ZIP containing the spritesheet PNG plus a TexturePacker-compatible JSON file. **Required** when `layout=packed`. |
...
### When `metadata_format=json_array`

Returns a ZIP archive containing `spritesheet.png` and `spritesheet.json`.

| Status | Content-Type | Body |
|---|---|---|
| `200` | `application/zip` | ZIP with PNG + JSON metadata |
